                                                   ...
                                                   Runnable
{
    ...
    static final String fServiceRequiredVersion = "3.5.9";
    ...
    // These are the "pretty" versions of the "Invalid Log Level Action"
    // combo box entries
    
    private static final String LOGINFO_PRETTY = new String("Log Info");
    private static final String RAISESIGNAL_PRETTY = new String("Raise Signal");
     
    private static final String[] INVALID_LOG_LEVEL_ACTIONS_PRETTY =
    {
        LOGINFO_PRETTY, RAISESIGNAL_PRETTY
    };
    
    // These are the valid values for the "INVALIDLOGLEVELACTION" option
    ...
    Vector<String> fBreakpointTableColumnNames;
    ...
    Vector<String> fActiveJobColumns;
    JMenu fFileMenu;
    JMenu fDisplayMenu;
    JMenu fHelpMenu;
    ...
    Vector<String> fRecentLogs = new Vector<String>();
    Vector<String> fRecentLogsTooltips = new Vector<String>();
    ...
    Vector<String> fScriptVector = new Vector<String>();
    Hashtable<String, STAXMonitorFrame> fMonitorTable =
        new Hashtable<String, STAXMonitorFrame>();
    ...
    Hashtable<String, Calendar> fJobStartTimes =
        new Hashtable<String, Calendar>();
    Hashtable<String, String> fJobStartDateTimes =
        new Hashtable<String, String>();
    Vector<String> fRecentFiles = new Vector<String>();
    ...
    Vector<String> fLocalExtJarFiles;
    Vector<String> fUpdatedLocalExtJarFiles;
    Vector fOldLocalExtJarFiles;
    Map<String, ExtensionInfo> fMonitorExtensionMap =
        new TreeMap<String, ExtensionInfo>();
    ...
    Vector<String> fTempLocalExtFiles = new Vector<String>();

    JList fPluginJarsList;

    // List of monitor extension plugin classes to be loaded by each job
    Vector<Class> fPluginClasses = new Vector<Class>();
    ...
    Vector<String> fExtensionsColumns;
    ...
    HashMap<String, Vector<Vector<String>>> fWizardFunctionMap;
    HashMap<String, String> fWizardFunctionTypeMap;
    HashMap<String, String> fWizardFunctionPrologMap;
    HashMap<String, String> fWizardFunctionEpilogMap;
    HashMap<String, JTable> fWizardFunctionArgTableMap;
    HashMap<String, Vector<String>> fWizardFunctionArgDefaultsMap;
    ...
    JComboBox fInvalidLogLevelActionCB = new JComboBox();
    ...
    Vector<String> fWizardSavedFunctionArgList = new Vector<String>();
    ...
    public STAXMonitor(String argv[])
    {
        ...
        // Resolve the {STAF/Config/Machine} variable on the local machine
        ...
        // Resolve the {STAF/Config/STAFRoot} variable on the local machine
        ...
        {
            JOptionPane.showMessageDialog(
                this, "Error resolving {STAF/Config/STAFRoot} " +
                "variable\n\nRC: " + result.rc + "\nResult: " + result.result,
                "Error resolving variable", JOptionPane.ERROR_MESSAGE);   
        }
        else
        {
            fSTAFRoot = result.result;
        }

        // Verify if the JSTAF.jar file contains a com.ibm.staf.STAFResult
        // class that contains a resultObj field (added for the STAF
        // auto-unmarshalling feature) to make sure that the STAX Monitor
        // is using a JSTAF.jar file for STAF V3.3.3 or later.
        
        try
        {
            Class<?> stafResultClass = Class.forName(
                "com.ibm.staf.STAFResult");
            stafResultClass.getDeclaredField("resultObj");
        }
        catch (Exception e)
        {
            System.out.println(
                "\nSTAX Job Monitor V" + fVersion +
                " requires the JSTAF.jar file for STAF version\n" +
                fSTAFRequiredVersion + " or later to be accessible via " +
                "the class path.\n\nIf you started the STAX Monitor " +
                "using the -jar option (e.g. java -jar STAXMon.jar), " +
                "make sure your STAXMon.jar file resides in a first " +
                "or second level directory off the STAF root directory" +
                " (e.g. " + fSTAFRoot + fFileSep + "services or " +
                fSTAFRoot + fFileSep + "services "+ fFileSep + "stax) " +
                "so that it can find the JSTAF.jar file in " + fSTAFRoot +
                fFileSep + "lib or " + fSTAFRoot + fFileSep + "bin.\n\n" +
                "Note that when using the -jar option, Java does not " +
                "use the CLASSPATH\nenvironment variable and does not " +
                "honor the -cp option.\n\n" + e.toString());
        ...
        fActiveJobColumns = new Vector<String>();
        ...
        fBreakpointTableColumnNames = new Vector<String>();
    ...
    public void loadExtensions()
    {
        ...
        for (Map.Entry<String, ExtensionInfo> entry :
             fMonitorExtensionMap.entrySet())
        {
            String extName = entry.getKey();
            ExtensionInfo extInfo = entry.getValue();
    ...
    public void createPropertiesDialog()
    {
        ...
        fExtensionsColumns = new Vector<String>();
        ...
        fLocalExtJarFiles = new Vector<String>();
        ...
        fPropertiesDialog.setSize(new Dimension(685, 380));
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        else if (e.getSource() == fPropertiesOKButton)
        {
            if (validateProperties())
            {
                ...
                fOldLocalExtJarFiles = new Vector<String>(fLocalExtJarFiles);
            }
        }
        ...
        else if (e.getSource() == fStartNewJobClearButton)
        {
            ...
            fWizardSavedFunctionArgList = new Vector<String>();
            ...
            fInvalidLogLevelActionCB.setSelectedItem(DEFAULT_STRING);
        ...
        else if (e.getSource() == fScriptDeleteButton)
        {
            ...
            if (deleteIndex > -1)
            {
                ...
                if (model.size() > deleteIndex)
                    model.removeElementAt(deleteIndex); 
             
                if (fScriptVector.size() > deleteIndex)
        ...
        else if (e.getSource() == fJobShowMonitorMenuItem)
        {
            synchronized(fMonitorTable)
            {
                ...
                if (rowIndex != -1)
                {
                    ...
                    {
                        STAXMonitorFrame monitorFrame = fMonitorTable.get(
                            jobNumber);
        ...
        else if (e.getSource() == fAddScriptFilesAddButton)
        {
            ...
            fScriptFilesVector.add(fAddScriptFilesTextField.getText());
        ...
        else if (e.getSource() == fWizardSaveButton)
        {
            ...
            String functionType = fWizardFunctionTypeMap.get(function);

            JTable table = fWizardFunctionArgTableMap.get(function);
            ...
            else if (functionType.equalsIgnoreCase("Single"))
            {
                ...
                String arguments = ((ArgumentValue)table.getValueAt(0, 3)).getString();
            ...
            else if (functionType.equalsIgnoreCase("List"))
            {
                ...
                Vector<String> argDefaults = fWizardFunctionArgDefaultsMap.get(
                    function);
                ...
                for (int i = 0; i < rows; i++)
                {
                    ...
                    if ( (required.equals("Yes")) ||
                         (required.equals("No") &&
                          !((argDefaults.get(i)).equals(value))) ||
    ...
    public void displayJobWizard()
    {
        ...
        fWizardFunctionMap = new HashMap<String, Vector<Vector<String>>>();
        fWizardFunctionTypeMap = new HashMap<String, String>();
        fWizardFunctionPrologMap = new HashMap<String, String>();
        fWizardFunctionEpilogMap = new HashMap<String, String>();
        fWizardFunctionArgTableMap = new HashMap<String, JTable>();
        fWizardFunctionArgDefaultsMap = new HashMap<String, Vector<String>>();
        ...
        while (functionIter.hasNext())
        {
            ...
            Vector<Vector<String>> functionArgs = new Vector<Vector<String>>();
            Vector<String> functionArgDefaults = new Vector<String>();
            ...
            while (argIter.hasNext())
            {
                ...
                Vector<String> argInfo = new Vector<String>();
            ...
            fWizardFunctionArgDefaultsMap.put(
                functionName, functionArgDefaults);
        }

        fWizardDialog = new JDialog(
            this, "STAX Job Wizard - " + fWizardFileName, true);
        ...
        while (iter.hasNext())
        {
            ...
            Vector<Vector<String>> functionVector = fWizardFunctionMap.get(
                function);
            ...
            {
                Vector<String> row = functionVector.elementAt(i);

                fObjectData[i][0] = new ArgumentName(row.elementAt(0));
                fObjectData[i][1] = new ArgumentDescription(row.elementAt(1));
                fObjectData[i][2] = new ArgumentRequired(row.elementAt(2));
                fObjectData[i][3] = new ArgumentValue(row.elementAt(3));
            }
            ...
            if (table.getRowCount() >= fWizardSavedFunctionArgList.size())
            {
                    ...
                    functionType.equals("Map"))
                {
                    for (int i = 0; i < fWizardSavedFunctionArgList.size(); i++)
                    {
                        table.setValueAt(
                            new ArgumentValue(
                                fWizardSavedFunctionArgList.elementAt(i)),
                            i, 3);
                    }
                }
            }
    ...
    public void updateWizardFunctionTable()
    {
        ...
        updateWizardFunctionType(fWizardFunctionTypeMap.get(function));
        ...
        fWizardFunctionArgsPanel.add(BorderLayout.CENTER,
            new JScrollPane(fWizardFunctionArgTableMap.get(function)));

        fWizardFunctionProlog.setText(fWizardFunctionPrologMap.get(function));
    ...
    public void displayJobWizardDetails()
    {
        ...
        fWizardFunctionDetails.setText(
            fWizardFunctionPrologMap.get(function) +
            fWizardFunctionEpilogMap.get(function));
    ...
    /**
     * Converts the "pretty" value for "Invalid Log Level Action", as displayed
     * by the STAX Monitor in the combo box, to a valid value for the
     * INVALIDLOGLEVELACTION option
     * @param action A string containing the "pretty" value for the
     * INVALIDLOGLEVELACTION option
     * @returns A string containing a valid value for the INVALIDLOGLEVELACTION
     * option
     */
    private static String getInvalidLogLevelActionFromPrettyString(String action)
    {
        if (action.equals(RAISESIGNAL_PRETTY))
            return RAISESIGNAL_STRING;
        else if (action.equals(LOGINFO_PRETTY))
    ...
    /**
     * Converts a valid value for the INVALIDLOGLEVELACTION option to the
     * "pretty" value for "Invalid Log Level Action", as displayed by the
     * STAX Monitor in the combo box
     * @param action A string containing a valid value for the
     * INVALIDLOGLEVELACTION option
     * @returns A string containing the "pretty" value for "Invalid Log Level
     * Action", that can be displayed by the STAX Monitor in the "Invalid Log
     * Level Action" combo box
     */
    private static String getPrettyInvalidLogLevelAction(String action)
    {
        if (action.equals(RAISESIGNAL_STRING))
            return RAISESIGNAL_PRETTY;
        else if (action.equals(LOGINFO_STRING))
            return LOGINFO_PRETTY;
        else
            return DEFAULT_STRING;
    }
    ...
    public boolean submitNewJob()
    {
        if (fMonitorNoRB.isSelected())
        {
            ...
            fScriptVector = new Vector<String>();
            ...
            String invalidLogLevelAction = getInvalidLogLevelActionFromPrettyString(
                (String)fInvalidLogLevelActionCB.getSelectedItem());
            
            if (!invalidLogLevelAction.equals(DEFAULT_STRING))
            {
                request.append(" INVALIDLOGLEVELACTION " + invalidLogLevelAction);
            }
        ...
        else
        {
            try
            {
                ...
                while (e.hasMoreElements())
                {
                    fScriptVector.add((String)e.nextElement());
                }
                ...
                while (e.hasMoreElements())
                {
                    fScriptFilesVector.add((String)e.nextElement());
                }
                ...
                String invalidLogLevelAction =
                    getInvalidLogLevelActionFromPrettyString(
                        (String)fInvalidLogLevelActionCB.getSelectedItem());
                
                Vector<String> breakpointTriggerVector = new Vector<String>();
                ...
                STAXMonitorFrame monitor = new STAXMonitorFrame(
                    ...
                    pythonOutput, pythonLogLevel, invalidLogLevelAction,
    ...
    public void monitorExistingJob(String jobNumber, int rowIndex)
    {
        if (fMonitorTable.containsKey(jobNumber))
        {
            STAXMonitorFrame monitorFrame = fMonitorTable.get(jobNumber);
            monitorFrame.setState(Frame.NORMAL);
            monitorFrame.toFront();
            monitorFrame.requestFocus();
            return;
        ...
        try
        {
            ...
            synchronized (fActiveJobsTable)
            {
                ...
                if (rowIndex == -1)
                {
                    ...
                    {
                        if (((Vector)(jobsVector.elementAt(j))).elementAt(0).
                            equals(new Integer(jobNumber)))
                        {
                            rowIndex = j;

                            // Set job's "Monitored" column to show it is
                            // being monitored
    ...
    public void createStartNewJobDialog()
    {
        ...
        fScriptScrollPane.setPreferredSize(new Dimension(450, 200));
        ...
        fAddScriptDialog = new JDialog(fStartNewJobDialog,
                                       "Add Script", true);
        fAddScriptDialog.setSize(new Dimension(450, 200));
        ...
        fEditScriptDialog = new JDialog(fStartNewJobDialog,
                                       "Edit Script", true);
        fEditScriptDialog.setSize(new Dimension(450, 200));
        ...
        fScriptFilesScrollPane.setPreferredSize(new Dimension(450,140));
        ...
        fAddScriptFilesDialog.setSize(new Dimension(450, 135));
        ...
        fEditScriptFilesDialog.setSize(new Dimension(450, 135));
        ...
        // Create "Invalid Log Level Action" Combo Box to be added to the
        // "Log Options" panel
        
        fInvalidLogLevelActionCB = new JComboBox();

        // Add "Default" as the first item in the "Invalid Log Level Action"
        // combo box, as the first item is the default selection if no
        // selection is made
        fInvalidLogLevelActionCB.addItem(DEFAULT_STRING);

        // Add all valid actions to the "Invalid Log Level Action" combobox
        for (int i = 0; i < INVALID_LOG_LEVEL_ACTIONS_PRETTY.length; i++)
        {
            fInvalidLogLevelActionCB.addItem(
                INVALID_LOG_LEVEL_ACTIONS_PRETTY[i]);
        }

        JPanel invalidLogLevelActionPanel = new JPanel();
        invalidLogLevelActionPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 0, 0));
        invalidLogLevelActionPanel.add(new JLabel("Invalid Log Level Action :"));
        invalidLogLevelActionPanel.add(Box.createHorizontalStrut(20));
        fInvalidLogLevelActionCB.setBackground(Color.white);
        fInvalidLogLevelActionCB.addActionListener(this);
        invalidLogLevelActionPanel.add(fInvalidLogLevelActionCB);
        ...
        logOptionsSubPanel.add(Box.createVerticalStrut(20));
        logOptionsSubPanel.add(invalidLogLevelActionPanel);
        ...
        fStartNewJobDialog.setSize(new Dimension(650, 375));
                                 ...
                                 boolean displayMessage)
    {
        ...
        {
            System.out.println(
                "Error reading the STAX Monitor Properties file (" +
                fPropertiesFileName +
                ").  This file may be corrupted.  If so, you can rename or " +
                "delete the properties file and restart the STAX Monitor " +
                "(which will generate a default properties file) to fix " +
                "the problem.  If you had customized the STAX Monitor " +
                "properties, you can re-enter any custom (non-default) " +
                "properties when the STAX Monitor is restarted.");
            e.printStackTrace();
            System.exit(0); 
        }
        ...
        Hashtable<String, Object> propertiesData =
        ...
        {
            System.out.println(
                "Error writing to the STAX Monitor Properties file (" +
                fPropertiesFileName + ").");
            ex.printStackTrace();
        }
    }
    ...
    public void saveProperties()
    {
        ...
        {
            System.out.println(
                "Error saving the STAX Monitor Properties file (" +
                fPropertiesFileName + ").");
            ex.printStackTrace();
        }
    }
    ...
    public void loadProperties()
    {
        try
        {
            ...
            // Load local extension jar files from job parms
            
            DefaultListModel pluginJarsModel =
                (DefaultListModel)fPluginJarsList.getModel();
            pluginJarsModel.removeAllElements();

            // Java can guarantee that the Object read back is a Vector of
            // some sort of Objects, just not what type of Objects.  To avoid
            // a unchecked cast warning, get the vector as Vector<Object>,
            // instead of Vector<String>.
            Vector tempVector = (Vector)propertiesData.get("pluginJars");

            if (tempVector != null)
            {
                fLocalExtJarFiles = new Vector<String>(tempVector.size());

                for (int i = 0; i < tempVector.size(); i++)
                {
                    String extJarFile = (String)tempVector.elementAt(i);
                    fLocalExtJarFiles.add(extJarFile);
                    pluginJarsModel.addElement(extJarFile);
                }
            }
            else
            {
            ...
            fUpdatedLocalExtJarFiles = new Vector<String>(fLocalExtJarFiles);
            fOldLocalExtJarFiles = new Vector<String>(fLocalExtJarFiles);
        ...
        {
            System.out.println(
                "Error loading the STAX Monitor Properties file (" +
                fPropertiesFileName + ").");
            e.printStackTrace();
            return;
        }
    ...
    public void saveJobParms(String fileName)
    {
        try
        {
            Hashtable<String, Object> jobParms =
                new Hashtable<String, Object>();
            ...
            Vector<String> scriptVector =
                new Vector<String>(scriptModel.getSize());

            for (int i = 0; i < scriptModel.getSize(); i++)
            {
                scriptVector.add((String)scriptModel.getElementAt(i));
            }
            ...
            Vector<String> scriptFilesVector =
                new Vector<String>(scriptFilesModel.getSize());

            for (int i = 0; i < scriptFilesModel.getSize(); i++)
            {
                scriptFilesVector.add(
                    (String)scriptFilesModel.getElementAt(i));
            }
            ...
            jobParms.put("invalidLogLevelAction",
                         getInvalidLogLevelActionFromPrettyString(
                             (String)fInvalidLogLevelActionCB.getSelectedItem()));
    ...
    public boolean loadJobParms(String fileName)
    {
        try
        {
            ...
            // a unchecked cast warning, get the vector as Vector<Object>,
            // instead of Vector<String>.
            Vector tempVector = (Vector)jobParms.get("scriptVector");

            if (tempVector != null)
            {
                fScriptVector = new Vector<String>(tempVector.size());

                for (int i = 0; i < tempVector.size(); i++)
                {
                    String script = (String)tempVector.elementAt(i);
                    fScriptVector.add(script);
                    scriptModel.addElement(script);
                }
            }
            else
            {
            ...
            // Load script files from job parms
            ...
            // a unchecked cast warning, get the vector as Vector<Object>,
            // instead of Vector<String>.
            tempVector = (Vector)jobParms.get("scriptFilesVector");
            
            if (tempVector != null)
            {
                fScriptFilesVector = new Vector<String>(tempVector.size());

                for (int i = 0; i < tempVector.size(); i++)
                {
                    String scriptFile = (String)tempVector.elementAt(i);
                    fScriptFilesVector.add(scriptFile);
                    scriptFilesModel.addElement(scriptFile);
                }
            }
            else
            {
            ...
            String invalidLogLevelAction = (String)jobParms.get(
                "invalidLogLevelAction");

            if (invalidLogLevelAction != null)
                fInvalidLogLevelActionCB.setSelectedItem(
                    getPrettyInvalidLogLevelAction(invalidLogLevelAction));
            else
                fInvalidLogLevelActionCB.setSelectedItem(
                    getPrettyInvalidLogLevelAction(DEFAULT_STRING));
            ...
            // Load function arg list for the Job wizard from job parms

            // Java can guarantee that the Object read back is a Vector of
            // some sort of Objects, just not what type of Objects.  To avoid
            // a unchecked cast warning, get the vector as Vector<Object>,
            // instead of Vector<String>.
            tempVector = (Vector)jobParms.get("wizardSavedFunctionArgList");

            if (tempVector != null)
            {
                fWizardSavedFunctionArgList =
                    new Vector<String>(tempVector.size());

                for (int i = 0; i < tempVector.size(); i++)
                {
                    fWizardSavedFunctionArgList.add(
                        (String)tempVector.elementAt(i));
                }
            }
            else
            {
            ...
            Vector breakpointLinesVector = (Vector)jobParms.get(
                "breakpointLinesVector");
    ...
    public void loadRecentFiles()
    {
        try
        {
            ...
            {
                fRecentFiles = new Vector<String>();
                return;
            }
            else
            {
                ...
                // Java cannot check that the object is actually a Vector
                // with an String type so instead of doing the following
                // which gets an unchecked cast warning
                //   fRecentFiles = (Vector<String>)(ois.readObject());
                // doing it this way:

                Vector tempVector = (Vector)(ois.readObject());
                ois.close();

                fRecentFiles = new Vector<String>(tempVector.size());

                for (int i = 0; i < tempVector.size(); i++)
                {
                    fRecentFiles.add((String)tempVector.elementAt(i));
                }
        ...
        if (!fRecentFiles.isEmpty())
        {
            ...
            for (int i=0; i < fRecentFiles.size(); i++)
            {
                ...
                fRecentFileMenuItems[i] =
                    new JMenuItem(fRecentFiles.elementAt(i));
    ...
    public void updateRecentLogs(String logQuery, String tooltip)
    {
        ...
        else
        {
            ...
            for (int i=0; i < 20; i++)
            {
                ...
                fRecentLogsMenuItems[i] = new JMenuItem(
                    fRecentLogs.elementAt(i))
                    {
                        public JToolTip createToolTip()
                        {
                            MultiLineToolTip tip = new MultiLineToolTip();
                ...
                fRecentLogsMenuItems[i].setToolTipText(
                    fRecentLogsTooltips.elementAt(i));
    ...
    public void registerForJobEvents()
    {
        ...
        STAFResult registerResult = fHandle.submit2(
            fEventMachineName, fEventServiceName, request);
        ...
        if (registerResult.rc != 0)
        {
            STAXMonitorUtil.showErrorDialog(
                ...
                "STAF " + fEventMachineName + " " + fEventServiceName +
    ...
    public void seedExistingJobs()
    {
        synchronized(fActiveJobsModelSorter)
        {
            ...
            {
                jobIDs.addElement(
                    (String)(((Vector)(jobsVector.elementAt(k))).elementAt(0)));
            }
    ...
    public void exit()
    {
        ...
        for (String jarFileName : fTempLocalExtFiles)
        {
    ...
    public Vector<Class> getPluginClasses()
    {
    ...
    private String convertMsgs(java.util.List<String> msgList)
    {
    ...
    {
        java.util.List<String> msgList = new ArrayList<String>();
        java.util.List<String> monitorExtList = new ArrayList<String>();
        ...
        for (String jarFileName : monitorExtList)
        {
            String localJarFileName = jarFileName;
    ...
    private void createExtensionJarFileList(java.util.List<String> monitorExtList,
                                   ...
                                   java.util.List<String> msgList)
    {
        ...
        while (mfIter.hasNext())
        {
            ...
            {
                ExtensionInfo extInfo = fMonitorExtensionMap.get(extName);
    ...
    class MonitorElapsedTime extends Thread
    {
        public void run()
        {
            ...
            while (fContinueElapsedTime)
            {
                ...
                final Enumeration<String> jobElapsedTimeKeys =
                    fJobStartTimes.keys();
                ...
                Runnable runnable = new Runnable()
                {
                    public void run()
                    {
                        ...
                        {
                            String jobID = jobElapsedTimeKeys.nextElement();

                            Calendar jobStarted = fJobStartTimes.get(jobID);
