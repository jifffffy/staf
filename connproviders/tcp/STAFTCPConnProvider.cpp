    ...
    try
    {
        ...
        *baseProvider = new STAFTCPConnectionProviderImpl(tcpData);
        ...
        if (tcpData.secure.isEqualTo(sYes, kSTAFStringCaseInsensitive))
        {
            ...
            // Specify TCL v1.2 protocol and disable other protocols like SSLv2
            // and SSLv3 to avoid vulnerabilities detected by TCP/IP scans

            (*provider)->server_ctx = SSL_CTX_new(TLSv1_2_server_method());

            if (!(*provider)->server_ctx)
                HANDLE_START_ERROR(ERR_reason_error_string(ERR_get_error()), 
                                   "SSL_CTX_new() server side");

            // Disable SSLv2, SSLv3, TLSv1, and TLSv1.1 protocols
            const long flags = SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 |
                               SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1;
                               
            if (SSL_CTX_set_options((*provider)->server_ctx, flags) <= 0)
                HANDLE_START_ERROR(ERR_error_string(ERR_get_error(), NULL),
                                   "SSL_CTX_set_options() server side");
            ...
            (*provider)->client_ctx = SSL_CTX_new(TLSv1_2_client_method());
            
            if (!(*provider)->client_ctx)
                HANDLE_START_ERROR(ERR_error_string(ERR_get_error(), NULL), 
                                   "SSL_CTX_new() client side");
                
            // Disable SSLv2, SSLv3, TLSv1, and TLSv1.1 protocols
            if (SSL_CTX_set_options((*provider)->client_ctx, flags) <= 0)
                HANDLE_START_ERROR(ERR_error_string(ERR_get_error(), NULL),
                                   "SSL_CTX_set_options() client side");                
