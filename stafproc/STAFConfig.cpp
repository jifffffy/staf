...
unsigned int readConfigFile(const STAFString &filename)
{
    ...
    for(; config.good() && !rc; lineNum++)
    {
        ...
        if (!config.good())
        {
            ...
            {
                cout << "Line " << lineNum
                     << " in configuration file exceeds maximum length of "
                     << STAFString(maxLineLength) << " characters" << endl;
                rc = 1;
                break;
            }

            if (strcmp(configLine, "") == 0) break;
        }
            ...
            (thisLine.subString(0, 1) == kUTF8_POUND))
        {
            ...
            {
                cout << "Line " << lineNum
                     << " is an invalid continuation line:" << endl << endl
                     << thisLine << endl;
                rc = 1;
            }
